package com.wpc.generator.provider.db.table;
...
public class TableFactory {
	...
	private Table _getTable(String catalog,String schema,String tableName) throws SQLException {
		...
		String[] types = {"TABLE"};
		ResultSet rs = dbMetaData.getTables(catalog, schema, tableName, types);
		while(rs.next()) {
	...
	private List<Table> getAllTables(Connection conn) throws SQLException {
		...
		// table type. Typical types are "TABLE", "VIEW", "SYSTEM TABLE", "GLOBAL TEMPORARY", "LOCAL TEMPORARY", "ALIAS", "SYNONYM".
		String[] types = {"TABLE"};
		ResultSet rs = dbMetaData.getTables(getCatalog(), getSchema(), null, types);
